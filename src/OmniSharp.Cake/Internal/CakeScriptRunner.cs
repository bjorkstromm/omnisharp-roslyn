...
namespace OmniSharp.Cake.Internal
{
    ...
    /// </summary>
    public sealed class CakeScriptRunner
    {
        ...
        public CakeScriptRunner(ICakeContext context)
            : this(context.Environment,
                  context.Log,
                  new CakeConfiguration(new Dictionary<string, string>()), //TODO: Get configuration
                  new CakeXPlatScriptEngine(null, context.Log),
                  new ScriptAliasFinder(context.Log),
                  new ScriptAnalyzer(context.FileSystem, context.Environment, context.Log, null),
                  new ScriptProcessor(context.FileSystem, context.Environment, context.Log, new ToolLocator(context.Environment, new ToolRepository(context.Environment), new ToolResolutionStrategy(context.FileSystem, context.Environment, context.Globber, new CakeConfiguration(new Dictionary<string, string>()))), new[] { new CakePackageInstaller() }),
                  new ScriptConventions(context.FileSystem, new AssemblyLoader(context.FileSystem, new AssemblyVerifier(new CakeConfiguration(new Dictionary<string, string>()), context.Log)), context.Log),
                  new AssemblyLoader(context.FileSystem, new AssemblyVerifier(new CakeConfiguration(new Dictionary<string, string>()), context.Log)))
        {
            _cakeContext = context;
        }
